...
export default function Home() {
  ...
  const categoryCounts = useMemo(() => {
    return {
      ...
      "10to30": searchedStocks.filter(
        (s) => !isETF(s) && s.price >= 10 && s.price < 30
      ).length,
      "30to50": searchedStocks.filter(
        (s) => !isETF(s) && s.price >= 30 && s.price < 50
      ).length,
      "50to100": searchedStocks.filter(
        (s) => !isETF(s) && s.price >= 50 && s.price < 100
      ).length,
      "100to200": searchedStocks.filter(
        (s) => !isETF(s) && s.price >= 100 && s.price < 200
      ).length,
      "200to500": searchedStocks.filter(
        (s) => !isETF(s) && s.price >= 200 && s.price < 500
      ).length,
      over500: searchedStocks.filter((s) => !isETF(s) && s.price >= 500)
        .length,
    };
  }, [searchedStocks]);
                ...
                <div className="top10List">
                  {top10Stocks.map((stock, index) => (
                    <div key={stock.symbol} className="top10Card">
                      ...
                      <div
                        className="stockMeta"
                        style={{ marginTop: 8, color: "#ffd36b" }}
                      >
                        推薦原因：{stock.reason || "--"}
                      </div>
                    </div>
